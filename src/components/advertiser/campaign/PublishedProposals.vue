    ...
    <v-layout class="mb-4" align-center>
      <v-flex sm8>
        <h5 class="subheading pl-3 mb-0">
          {{ proposals.length }} Published Posts
    ...
    <v-card class="grey lighten-4 elevation-1" style="margin-bottom: 300px" v-if="!proposals || proposals.length === 0">
      <v-card-text>
        <v-layout align-center justify-center>
          <div class="subheading mt-3 mb-3" style="font-weight: 300">Currently there are no posts.</div>
        </v-layout>
      </v-card-text>
    </v-card>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.$http.get(`campaign/${this.campaign.id}/proposals`, { params: { 'status': 'published' }}).then(result => {
