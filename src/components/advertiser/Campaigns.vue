<template>
  <div class="main-content">
    <v-layout row align-center class="mb-4">
      <v-flex sm13>
        <h4 class="subheading">
          <v-icon class="primary--text mr-3">view_list</v-icon> <strong>All Campaigns</strong>
        </h4>
      </v-flex>
      <v-flex sm3 class="text-sm-right">
        <v-btn
            color="purple"
            dark
            tag="a"
    ...
    <v-toolbar dense class="white pa-0 elevation-1 mb-5">
      <v-layout row align-center>
        <v-flex sm12>
          <v-tabs class="tabs--condensed white elevation-0" v-model="filter">
            <v-tab href="#live">
              <v-icon class="red--text">inbox</v-icon>
              Live
            </v-tab>
            <v-tab href="#finished">
              <v-icon class="primary--text">done_all</v-icon>
              Finished
            </v-tab>
            <v-tab href="#draft">
        ...
        <v-flex sm6 class="pr-4">
          <v-text-field append-icon="search" hide-details single-line placeholder="Search campaigns..." v-model="search">
    ...
    <v-data-table
        ...
        :headers="headers"
        no-data-text="There are no campaigns"
        style="z-index: 4"
    >
      ...
      <template slot="items" slot-scope="props">
        ...
        <td class="text-sm-center">
          <toggle-button :value="props.item.status === 10"
                         ...
                         :height="30"
                         :sync="true"
                         v-if="props.item.status === 10 || props.item.status === 5"
...
<script>
  export default {
    ...
    computed: {
      tabColor: function () {
        if (this.filter === 'live') return "red"
        if (this.filter === 'finished') return "primary"
        if (this.filter === 'draft') return "orange"
      }
    },
    watch: {
      filter: function () {
    ...
    methods: {
      fetchCampaigns() {
        ...
        this.$http.get('campaigns', { params: { 'status': this.filter }}).then(result => {
...
<style>
  ...
  .campaign--list .table tbody tr td.pa-0 {
    padding: 0 !important;
