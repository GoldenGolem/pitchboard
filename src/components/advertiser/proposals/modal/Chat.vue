  ...
  <div class="white elevation-1" style="margin-top: -46px">
    ...
    <form @submit.stop.prevent="sendMessage" class="pa-0">
      <v-card-actions class="pa-0" style="border-top: 1px solid #eeeeee">
        <v-layout row class="white ma-0" justify-center align-center>
          <v-flex sm13>
            <v-text-field
          ...
          <v-flex sm3 class="mr-1 ml-3">
            <v-btn fab small flat class="grey--text" @click.native.prevent.stop><v-icon style="font-size: 26px">attachment</v-icon></v-btn>
  ...
  export default {
    ...
    methods: {
      fetchMessages() {
        this.$http.get(`/messages/${this.proposal.id}`).then(result => {
          ...
          if (this.messages.length > 0) {
            this.markRead(this.messages[0].conversation_id)
          }
        })
      },
      ...
      markRead(conversation_id) {
        this.$http.get(`conversation/${conversation_id}/read`).then(result => {
