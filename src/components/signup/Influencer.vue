    ...
    <v-layout row justify-center>
      <v-flex md8>
        ...
        <div class="text-xs-center mb-4">
          ...
          <p class="grey--text text--darken-1">To View the campaign details sign in below</p>
        </div><!-- /.sign-up-header -->

        <v-card raised style="z-index: 2" class="mb-3">
          <v-card-text class="pa-5" transition="slide-y-transition" v-if="invitation_id">

            <form method="post" action="" class="form-sign-up">

                <h5 class="title text-xs-center">Login with social media</h5>
          ...
          <v-card-text class="pa-5" v-else>
            <h5 class="title text-xs-center mb-3">Enter your invitation code</h5>
            ...
            <v-layout row justify-center>
              <v-flex sm10>
                ...
                <v-btn block color="primary" @click="verifyInvitation">
                  <v-icon left>check</v-icon>
                  Apply Code
                </v-btn>

                <div class="mt-4 caption grey--text text--darken-2">
  ...
  export default {
    ...
    methods: {
      getStartedDone(data) {
        ...
        this.user = {...data.user}
        this.step = 'connect-networks'
      },
      connectNetworksDone(data) {
        this.influencer = {...data.influencer}
        this.user = {...data.user}
        this.step = 'connect-networks'
      },
      verifyInvitation(showAlert = true) {
        this.$http.post('signup/influencer/verify-invitation', { code: this.invite_code }).then(result => {

          this.invitation_id = result.data.invitation_id

          if (showAlert) this.$store.dispatch('alert', {
            'text': 'Invitation code successfully applied',
            'color': 'success'
          })
          ...
          if (error.response.status === 422) {
            this.$store.dispatch('alert', {
              'text': 'You have to provide the invitation code',
              'color': 'error'
            })
          } else if (error.response.data && error.response.data.error === 'not_valid') {
            this.$store.dispatch('alert', {
              'text': 'Invitation code not valid',
              'color': 'error'
            })
          } else {
            this.$store.dispatch('alert', {
      ...
      authenticate(provider, options) {
        this.$auth.authenticate(provider, options).then(result => {
          ...
          if (result.data.login) {
            this.login(result.data.token, false, '/influencer')
            ...
            this.influencer.name = this.social.name
          }
        }).catch(error => {
          console.log(error)
        })
      ...
      submit() {
        this.$validator.validateAll().then(result => {
          ...
          this.$http.post('signup/influencer', {
              'user': this.user,
      ...
      login(token, signup = true, redirect = '/influencer/start') {
